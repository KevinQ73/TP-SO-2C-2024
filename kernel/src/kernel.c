...
int main(int argc, char* argv[]) {
    ...
    t_pcb* create_pcb(){
        ...
        PCN->mutex_asociados = list_create();
    ...
    void* peticion_crear_proceso(void){
        t_pcb* pcb = create_pcb();
        poner_en_new(pcb);
        log_debug(kernel_log,"PID: %d- Se crea el proceso- Estado: %s",estado_proceso->NEW_STATE, pcb->pid);
